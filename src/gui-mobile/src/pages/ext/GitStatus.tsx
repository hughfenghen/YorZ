...
import { api, type GitChange } from '@shared/api/index.js'
import { subscribeProjectChanges } from '@shared/api/sse.js'
 ...
 * 2. **没有 Agent 派发模式**：桌面端那条支路要 specId 才能 `api.gitOp`，
 *    从扩展页进来的这一页没有 spec 上下文，桌面端在无 specId 时同样整段不渲染。
 *    所以这里只有 direct 分支，等价于桌面端的 `fileSelectMode === 'manual'`。
 */
export const GitStatus: Component = () => {
  ...
  const canCommit = createMemo(
    () => busy() === null && selected().size > 0 && message().trim().length > 0,
  )
  ...
  const moreItems = (): ActionSheetItem[] => {
    if (confirmingDiscard()) {
      return [{ label: t('git.discardConfirm'), tone: 'destructive', onSelect: () => void discard() }]
    }
        ...
        <Show when={activeProjectId()}>
          <div class="shrink-0 border-t border-border bg-card px-4 py-2 px-safe pb-safe">
            ...
            <div class="mt-2 flex items-center gap-2">
              ...
              >
                {busy() === 'discard' ? t('git.discarding') : t('git.discard')}
              </button>
              <button
                type="button"
    ...
    >
      <Show when={activeProjectId()} fallback={<NoProjectNotice />}>
        ...
        <button
          ...
          disabled={busy() !== null || branchState.loading}
          onClick={() => {
            setBranchQuery('')
            setBranchSheet(true)
          }}
      ...
      <ActionSheet
        open={moreOpen()}
        description={confirmingDiscard() ? t('git.discardHint') : undefined}
        items={moreItems()}
        onClose={() => {
          setMoreOpen(false)
          setConfirmingDiscard(false)
      ...
      >
        ...
        <Show
          ...
          fallback={<p class="py-6 text-center text-sm text-muted-foreground">{t('git.noBranches')}</p>}
        >
              ...
              {(branch) => (
                <li>
                  <button
                    ...
                    disabled={busy() !== null || branch === branchState()?.current}
...
const DiffBody: Component<{
  ...
  return (
    ...
    >
      <Show when={!props.error} fallback={<p class="p-4 text-sm text-destructive">{props.error}</p>}>
        <Show
          when={!props.binary}
