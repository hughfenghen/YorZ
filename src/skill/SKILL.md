...
- 若以下章节不存在，先补齐再继续：
  ...
  - `## 追加任务`
  - `## 执行记录`
- `## 追加任务` 章节位置固定在 `## 任务清单` 之后、`## 执行记录` 之前；spec-store 端 `SECTIONS` 须与本约束一致。
...
2. 否则，**优先扫描 `## 追加任务` 中是否存在 `[open]` 条目**：存在即视为"新增/扩展需求"或"新增 bug"信号，按下一条判定进入 `plan`（重开流程）。
3. 否则，若识别到新增/扩展需求或新增 bug，进入 `plan`（重开流程）。
4. 否则，若文档存在任意 `！！！` 批注，进入 `tasks`。
5. 否则，若 `## 待确认问题` 下存在有效条目（非 `- 暂无`），停止推进并等待人工批注。
6. 否则，若存在未完成任务（`- [ ]`），进入 `execute`。
7. 否则，若 `## 技术实现方案` 为空或明显不完整，进入 `plan`。
8. 否则，进入 `tasks` 生成或刷新详细任务清单。
...
- **候选项硬约束**：每条问题**若**给出候选项，则必须**恰好** 1 个候选项以 ` (推荐)` 结尾；候选数量不做强制下限。无法或不必枚举（如概念澄清、错别字确认、开放性补全）时，在问题正文末尾追加 `（自由文本）` 后缀，显式声明该条退化为自由文本条目（详见《待确认问题结构》）。
...
- 若本轮起因是 `## 追加任务` 中的 `[open]` 条目：每完成一个追加项，必须把对应条目的状态标记从 `[open]` 改为 `[fixed]`（位置和正文保留不变），并在 `## 执行记录` 追加一条结果（链接到该条目的描述首行）；已 `[fixed]` 的条目保留在 md 中作为历史，不删除。
