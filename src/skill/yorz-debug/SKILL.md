...
- `spec_dir`（**可选**）：spec 目录路径（含 `spec.md`）。给出时 `debug.md` 建在此目录，与 `spec.md` 同级。
  ...
  - 其中的 `@` 只是 YorZ 的文件引用标记，**不属于路径本身**；去掉 `@` 后是**项目根目录**（即当前工作目录）下的相对路径，不要相对本 skill 文件所在目录或用户主目录解析。
...
5. **人机协作**：优先主动复现 Bug、获取日志进行分析判断，如果无途径复现 Bug、获取日志，也可暂停执行，让用户配合复现 Bug、回传日志。
...
- **可定位**：日志带上下文（哪个函数 / 哪个分支 / 关键变量值），便于判定假设成立与否。
