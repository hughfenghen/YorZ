...
/**
 * Error copy, injected by the host app.
 *
 * The controller lives in `gui-shared` and therefore cannot reach either app's
 * `t()`: desktop and mobile own separate dictionaries. Passing the five strings
 * in keeps the controller free of i18n while letting each side keep its own keys.
 */
export interface AttachmentLabels {
  /** All slots taken; nothing was accepted. */
  attachmentLimit: (max: number) => string
  /** File kind not in the allow-list. */
  unsupportedType: (name: string) => string
  /** Kind looked fine but the MIME is not one we accept. */
  unsupportedMime: (mime: string) => string
  fileTooLarge: (name: string) => string
  /** Some files were accepted, the rest dropped for lack of room. */
  attachmentRoomLimit: (room: number, current: number, max: number) => string
}
...
export function createAttachments(opts: {
  ...
  async function addFiles(files: File[]) {
    ...
    if (room <= 0) {
      setError(labels.attachmentLimit(MAX_COUNT))
      return
    }
    ...
    if (files.length > room) {
      setError(labels.attachmentRoomLimit(room, current.length, MAX_COUNT))
    }
