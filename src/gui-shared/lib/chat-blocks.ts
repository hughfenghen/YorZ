import type { AgentContextKind, AgentKind, MessagePart, SessionMessage } from '../api/index.js'
...
/**
 * Expand state for every collapsible in the tool tree, owned by the transcript
 * hook rather than by the tool component.
 *
 * It cannot live inside that component. `groupParts` allocates new objects on
 * every stream tick, so the message list's reconciliation tears those instances
 * down and rebuilds them several times a second while a session runs — an
 * instance-local signal was reset to `false` under the reader's cursor. Keyed by
 * the segment's stable id (see `ToolsSegment.id`), the state outlives both the
 * re-render and a mid-run transcript re-read.
 */
