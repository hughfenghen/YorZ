...
export const RadioGroupItemControl = <T extends ValidComponent = 'div'>(
  ...
  return (
    <RadioGroupPrimitive.ItemControl
      class={cn(
        'flex aspect-square h-4 w-4 items-center justify-center rounded-full border border-input bg-background shadow-sm transition-[border-color,background-color,box-shadow] focus:outline-none focus-visible:ring-[1.5px] focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50 data-[checked]:border-primary data-[checked]:bg-primary data-[checked]:shadow data-[checked]:ring-2 data-[checked]:ring-primary/20',
        local.class,
      )}
      {...rest}
    >
      <RadioGroupPrimitive.ItemIndicator class="h-2 w-2 rounded-full bg-primary-foreground" />
    </RadioGroupPrimitive.ItemControl>
  )
}
