...
- **事件模型**：稳定信封 + 开放事件名。信封为 `{ v, ts, event, projectId, traceId?, durMs?, ...payload }`（schema `v: 2`，`ts` 为毫秒时间戳数字），`event` 采用 `<域>.<动作>` 两段式；新增可观测点只需定义新的事件字符串，核心模块零改动。
- **事件集**：`agent.dispatch`（派发起止、trigger、耗时、成败）、`agent.turn`（归一化 token 用量 / 成本 / 模型分桶）、`agent.compact`（上下文压缩前后 token）、`spec.stage`（阶段迁移与任务完成度）、`cmd.exec`、`lint.run`。`git.op` 与 `spec.change` 曾存在于 v1，因占据 99% 行数却不喂养任何指标而移除。
...
- **轮内阶段切分**（claude adapter）：`agent.turn` 额外带 `planPhase` 与 `observedTotal` 两个 `PhaseUsageSnapshot`（`{ usage, requests, durationMs }`）。Agent 只在轮末报一次总量，无法回答「这次派发有多少花在 plan 上」；因此 adapter 逐条累加 assistant 消息自带的单次请求 usage，并在**首次写 `spec.md`**（`Write`/`Edit`/`MultiEdit`/`NotebookEdit` 且 `file_path` 以 `spec.md` 结尾）时打快照 `planPhase`——写回 spec 是 plan 的产物，故该条请求计入 plan 段；一轮内多次写只认首次。`observedTotal` 是**同一累加器**在轮末的读数：`usage` 字段来自 Agent 自己的轮末汇总，与逐请求累加是两套口径，直接相除会把口径差伪装成阶段差，故 `planPhase / observedTotal` 才是同源可比的比值，再乘 `usage.costUsd` 折算成金额。未写过 spec 的派发（git 操作、explain、纯对话）`planPhase` 缺席，行形状与既有数据完全一致。已知缺口：按路径后缀匹配，测不到 `docs/specs/<name>.md` 这类存量路径。
- **落盘位置**：**全局单文件** `<globalConfigDir>/metrics/telemetry.jsonl`（与 `logs/` 同级，遵循 `YORZ_HOME` > `XDG_CONFIG_HOME` > `~/.config` 解析）。所有项目共写这一个文件，靠行内 `projectId`（即 `generateProjectId(项目绝对路径)`）区分；同目录 `projects.json` 记录 id → 路径索引。数据不进入项目工作区，因此无需改动 `.gitignore`。
- **保留策略**：保留最近 365 天。Service 启动时执行一次 `initTelemetryStore()`：把历史遗留的 `metrics/<projectId>/` 子目录合并进统一文件（顺带把 v1 的字符串 `ts` 转为数字、丢弃已废弃的事件类型），再按保留期裁剪并按 `ts` 升序原子重写。不再有按大小轮转——单文件轮转会跨项目连带截断，也无法表达「保留一年」。
- **零侵入**：复用 `RotatingFileSink`（入队即返、单链串行、错误全吞；轮转阈值置为无穷即关闭），且所有项目共享同一个 sink 实例以保证行不交错；埋点失败或磁盘异常绝不影响主流程。
