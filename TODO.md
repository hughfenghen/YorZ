...
未渲染到轨道区的素材（elements），不参与初始化子轨道计算，比如mock数据中有许多 shot 已被注释，意味着这些shoot以及对应的素材都不会渲染到轨道，这些素材不参与子轨道计算。@editor/app/components/cut/mock-data.ts:L5-L39
轨道区的素材元素被拖拽，应该立刻更新轨道状态，而不是等到松开鼠标，再切换到新轨道
```
